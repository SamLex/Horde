...
public class HordeCommand implements CommandExecutor{
	...
	/*
	 * Sets up and starts the timers that execute the spawns. Used to allow the timers to be restarted after a configuration reload
	 */
			...
			if(this.inst.getConfig().getBoolean("multi")){
				...
				while(iter.hasNext()){
					...
					if(this.inst.getConfig().contains(w.getName())){
						...
						this.inst.getServer().getScheduler().scheduleAsyncRepeatingTask(inst, hordeSpawner, time, time);
						hordeSpawner = null;
					}else{
