...
public class Horde extends JavaPlugin {
    ...
    /*
     * Runs when the plugin is enabled. Instances of the entity listener and the
     * command handler are created and then registered. The timers used for the
     * spawns are then started. A ready message is
     * then printed
     */
    @Override
    public void onEnable() {
        ...
        this.config();
        this.getServer().getPluginManager().registerEvents(hel, this);
    ...
    /*
     * Runs just as the plugin is disabled. Cancels the timers to stop them from
     * running after the plugin has been disabled. Saves any changes to the
     * configuration file to disk and then prints a
     * goodbye message
     */
    @Override
    ...
    /*
     * Checks that all the worlds loaded are in the configuration file and if
     * not they are added. The configuration file is then saved to disk
     */
    private void config() {
